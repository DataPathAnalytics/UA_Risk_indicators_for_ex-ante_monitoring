...
@Service
@Slf4j
public class Risk_2_6_Extractor extends BaseExtractor {
    ...
    */
    ...
    public Risk_2_6_Extractor(NearThresholdRepository nearThresholdRepository) {
    ...
    public void checkIndicator(ZonedDateTime dateTime) {
        try {
            ...
            if (nonNull(indicator) && tenderRepository.findMaxDateModified().isAfter(ZonedDateTime.now().minusHours(AVAILABLE_HOURS_DIFF))) {
                checkRisk2_6Indicator(indicator, dateTime);
            }
        } catch (Exception ex) {
    ...
    private void checkRisk2_6Indicator(Indicator indicator, ZonedDateTime dateTime) {
        int size = 100;
        int page = 0;
        while (true) {
