...
@Service
@Slf4j
public class Risk_2_5_1_AprilExtractor extends BaseExtractor {

    private final String INDICATOR_CODE = "RISK-2-5-1";
    private boolean indicatorsResolverAvailable;
    private NearThresholdRepository nearThresholdRepository;

    public Risk_2_5_1_AprilExtractor(NearThresholdRepository nearThresholdRepository) {
    ...
    @Async
    @Scheduled(cron = "${risk-2-5-1.cron}")
    ...
    private void checkIndicator(Indicator indicator, ZonedDateTime dateTime) {
        ...
        while (true) {
            ...
            List<Object[]> tenders = tenderRepository.getIndicator2_5_1TenderData(
                    dateTime,
            ...
            List<TenderIndicator> tenderIndicators = tenders.stream().map(tenderInfo -> {
                ...
                try {
                    ...
                    if (isNull(amount)) {
                        ...
                        indicatorValue = amount >= 1000000 ? RISK : NOT_RISK;
                    }
                } catch (Exception e) {
